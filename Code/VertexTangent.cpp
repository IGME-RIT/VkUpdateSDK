...
// three attributes
static VkVertexInputAttributeDescription vertexInputAttributs[4];
...
VkPipelineVertexInputStateCreateInfo VertexTangent::GetState()
{
	...
	vertexInputBinding.stride = sizeof(VertexTangent);
	...
	memset(vertexInputAttributs, 0, sizeof(VkVertexInputAttributeDescription) * 4);
	...
	vertexInputAttributs[0].offset = offsetof(VertexTangent, position);
	...
	vertexInputAttributs[1].offset = offsetof(VertexTangent, uv);
	...
	vertexInputAttributs[2].offset = offsetof(VertexTangent, normal);
	...
	// location = 3, because this is the forth element of the vertex
	vertexInputAttributs[3].location = 3;

	// offsetof returns 32, because tangent starts 32 bytes after the start of the vertex,
	// because there are 32 bits in position, UV, and Normal combined
	vertexInputAttributs[3].offset = offsetof(VertexTangent, tangent);
	...
	// We tell it how many attributes there are (four) (pos, uv, normal, tangent),
	// then we give it the array of four attributes (which is already a pointer)
