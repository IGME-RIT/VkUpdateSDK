...
{
	vec2 uv;
	if (gl_VertexIndex == 0) uv = vec2(1, 0);
	if (gl_VertexIndex == 1) uv = vec2(0, 0);
	if (gl_VertexIndex == 2) uv = vec2(1, 1);
	if (gl_VertexIndex == 3) uv = vec2(0, 1);
	...
	else
	{
		...
		// Remember to flip the Y axis. Make the zeros
		// into ones, and make the ones into zeros
		// (in comparison to OpenGL), to adjust for Vulkan's
		// flipped axis. This can also be fixed by using 
		// Vulkan 1.1 and a negative viewport, but I want
		// this tutorial to work for everyone, including 
		// the few people that might have Vulkan 1.0 GPUs
		// that don't support 1.1
